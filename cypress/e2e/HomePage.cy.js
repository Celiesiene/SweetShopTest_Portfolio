describe('Home Page Test', () => {
  it('Home Page funcionality', () => {
    ...
    // cy.get('.row').should('have.length.greaterThan', 1); kaip padaryt, kad veiktų?
    // cy.get('.messageContainer h2').contains('Most popular').should('be.visible'); //veikia, testas turi nepraeiti
  ...
  it('1.5 - Verify that clicking “Browse sweets” displays the full product list', () => {
    ...
    cy.get('.row').should('have.length.greaterThan', 1); // čia atskiri div
    cy.get('.row').each(($row) => {
...
//Klaidos: 
//.messageContainer h2 Most popular n4ra matomas
